...
const useInfinitePaginatedQuery = (
  ...
  data: Parameters<typeof useGetDataListQuery>[1];
  page: number;
  hasMore: boolean;
  fetchMore: () => void;
  refresh: () => void;
  ...
  const queryResponse = useGetDataListQuery(
    {
      ...localParams.current,
      limit,
      ...(hasMetadata ? { from: localPage.id } : { offset: (localPage.page - 1) * limit }),
    },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    queryOptions as any,
  );

  type InfiniteQueryResponseType = InfiniteQueryResponse<typeof useGetDataListQuery>;
  ...
  return {
    ...queryResponse,
    data: combinedData.slice(pageIndex * limit, (pageIndex + 1) * limit),
    page: localPage.page,
