...
describe('Auctions', () => {
  ...
  it('should support the main flow', async () => {
    ...
    expect(auctionAfter1BidAlice.currentBid).toBe('100');
    ...
    expect(auctionAfter1BidBob.currentBid).toBe('100');

    await makeBid(auction, aliceToken);
    const auctionAfter2BidAlice = await getFirstAuction(aliceToken);
    expect(auctionAfter2BidAlice.currentBid).toBe('101');
    ...
    expect(auctionAfter2BidBob.currentBid).toBe('101');

    await makeBid(auction, bobToken);
    const auctionAfter3BidsAlice = await getFirstAuction(aliceToken);
    expect(auctionAfter3BidsAlice.currentBid).toBe('102');
